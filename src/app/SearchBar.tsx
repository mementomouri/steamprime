    ...
    <div className="flex-1 max-w-md" ref={searchRef}>
      <div className="relative group">
        <input
          ...
          className="w-full h-10 sm:h-12 px-4 sm:px-5 pr-10 sm:pr-12 bg-white/90 backdrop-blur-sm border-2 border-blue-100 rounded-lg sm:rounded-xl text-gray-700 placeholder:text-gray-400 text-sm font-medium shadow-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white focus:shadow-md transition-all duration-200 outline-none hover:border-blue-200 hover:shadow-sm"
          style={{ direction: 'rtl' }}
        />
        <div className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6">
          {isSearching ? (
            <svg className="w-3 h-3 sm:w-4 sm:h-4 text-blue-500 animate-spin" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
          ) : (
            <svg 
              className="w-3 h-3 sm:w-4 sm:h-4 text-blue-500 group-hover:text-blue-600 transition-colors duration-200" 
              fill="none" 
              stroke="currentColor" 
              strokeWidth="2" 
              viewBox="0 0 24 24"
        ...
        {/* افکت فوکوس */}
        <div className="absolute inset-0 rounded-lg sm:rounded-xl bg-gradient-to-r from-blue-500/5 to-blue-600/5 opacity-0 group-focus-within:opacity-100 transition-opacity duration-200 pointer-events-none"></div>
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-sm border border-blue-200 rounded-lg shadow-xl z-50 max-h-80 overflow-y-auto">
            {isSearching ? (
              <div className="p-3 sm:p-4 text-center">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 animate-spin mx-auto mb-2" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                <span className="text-xs sm:text-sm text-gray-600">در حال جستجو...</span>
              </div>
              ...
              <div className="py-2">
                {searchResults.map((result, index) => (
                  <div 
                    key={index}
                    className="px-3 sm:px-4 py-2.5 sm:py-3 hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                    onClick={() => {
                      setSearchTerm('');
                      setShowDropdown(false);
                    }}
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="font-semibold text-gray-800 text-xs sm:text-sm">{result.productName}</div>
                      ...
                      <div className="text-left mr-2 sm:mr-3">
                        <div className="font-bold text-red-600 text-xs sm:text-sm">
                ...
                {allSearchResults.length > 5 && (
                  <div 
                    className="px-3 sm:px-4 py-2.5 sm:py-3 bg-blue-50 hover:bg-blue-100 cursor-pointer border-t border-blue-200"
                    onClick={handleShowAllResults}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs sm:text-sm font-semibold text-blue-700">نمایش همه نتایج</span>
                      <span className="text-xs text-blue-600 bg-blue-200 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">
              ...
              <div className="p-3 sm:p-4 text-center">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 text-gray-400 mx-auto mb-2" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                </svg>
                <span className="text-xs sm:text-sm text-gray-600">نتیجه‌ای یافت نشد</span>
              </div>
            )}
          </div>
        )}
